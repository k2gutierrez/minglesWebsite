...
import { ton_curtis_address, ton_address, TON_ABI } from '@/app/ton-stickers-ape/TON';
...
const SocialLinksTrigger = ({ triggerText = "Follow Us" }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative inline-block text-left">
      ...
      <div
        className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-3 flex flex-col items-center gap-3 transition-all duration-300 ease-in-out z-10 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'
          }`}
        role="menu"
...
export function MintComponent2() {

  const { writeContract, data: hash } = useWriteContract();
  const { data: receipt, isSuccess: mintSuccess } = useWaitForTransactionReceipt();
  ...
  let tonApeAddress: string;

  if (chainId == 33111) {
    tonApeAddress = ton_curtis_address;
  } else {
    tonApeAddress = ton_address;
  }

  const { data: supply } = useReadContract({
    abi: TON_ABI,
    address: tonApeAddress as `0x${string}`,
  ...
  const userWallet = mainUser?.wallet;

  const {data: totalPurchased, refetch: getPurchased} = useReadContract({
    abi: TON_ABI,
    address: tonApeAddress as `0x${string}`,
    functionName: 'getTotalPurchased',
  })
  
  const {data: purchaseAmount} = useReadContract({
    abi: TON_ABI,
    address: tonApeAddress as `0x${string}`,
    functionName: 'getNormalPrice',
  })
  ...
  useEffect(() => {
    ...
    if (isConnected) {
      setIsLoading(true);
      console.log(userWallet?.toLowerCase() == address?.toLowerCase())
      if (userWallet?.toLowerCase() == address?.toLowerCase()) {
  ...
  const handleMint = async () => {
    if (tonWallet == "") {
      alert("No tonWallet registered yet!")
      return;
    }

    writeContract({
      address: tonApeAddress as `0x${string}`, // Your deployed contract address
      abi: TON_ABI,
      functionName: 'purchaseTonStickerAndEnterRaffle', // The function in your contract
      args: [tonWallet], // Arguments for the function
      value: purchaseAmount as bigint, // Optional: value in Ether to send
    });
  ...
  const renderContent = () => {
    ...
    if (isHolder) {
      return (
        <>
          ...
          <input
            className="placeholder:text-gray-500 placeholder:italic ... w-100 p-1 mx-1 mb-1 mt-5 border border-solid rounded-lg border-blue-500"
          ...
          />
          <button
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-3 px-6 mt-2 rounded-lg text-lg w-full"
            onClick={handleMint}
          >
            Mint 10-Pack using ape on apechain
          </button>
        </>
      );
    }
    if (isConnected) {
      return (
        <>
        <input
            className="placeholder:text-gray-500 placeholder:italic ... w-100 p-1 m-1 border border-solid rounded-lg border-blue-500"
            placeholder="Enter TON wallet"
            type="text"
            onChange={(e) => {
              setTonWallet(e.target.value)
  ...
  return (
    ...
    <div className="w-full max-w-md mx-auto h-screen sm:h-auto flex-col justify-start p-6 text-black">
      ...
      <div id='divToAlignToLeft' className={`w-full flex justify-normal space-x-2 mb-2 ${vietnamLight.className}`}>
        ...
        <div className="text-xs md:text-sm font-bold text-gray-500 bg-gray-100 rounded-full px-1 mr-30 sm:mr-65">MINTED<span className='ms-1 text-black'>{supply ? `${Number(totalPurchased)}/${Number(supply)}` : "0/0"}</span></div>
      </div>
