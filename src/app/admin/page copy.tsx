...
export default function AdminPage() {
    ...
    // Nuevo estado para mostrar si está cargando la imagen
  const [isUploading, setIsUploading] = useState(false);
    ...
    // Al cargar la página, traemos los catálogos
    useEffect(() => {
    ...
    const renderFormFields = () => {
        switch (activeTab) {
            ...
            case 'game_raids':
                // Asegurarnos de que yield_config exista como objeto en el form
                const yields = editForm.yield_config || { "7": { min: 0, max: 0 }, "15": { min: 0, max: 0 }, "30": { min: 0, max: 0 } };
                // Asegurarnos de que loot_table exista como arreglo
                ...
                return (
                    <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        ...
                        {/* SELECTOR DE BOSS (Visual) */}
                        <div>
                            <label className="text-xs font-bold text-gray-500">Seleccionar Jefe (Boss)</label>
                            <select value={editForm.boss_id || ''} onChange={e => setEditForm({ ...editForm, boss_id: parseInt(e.target.value) })} className="border p-2 rounded w-full">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <select value={editForm.difficulty || 'Easy'} onChange={e => setEditForm({ ...editForm, difficulty: e.target.value })} className="border p-2 rounded w-full">
                                    <option value="Easy">Easy</option><option value="Medium">Medium</option><option value="Hard">Hard</option>
                                </select>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-gray-500">Color Theme (Tailwind)</label>
                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border">
                            ...
                            {(['7', '15', '30'] as const).map(days => (
                                <div key={days} className="flex items-center gap-2 mb-2">
                                    <span className="w-16 font-bold text-xs">{days} Días:</span>
                                    <input type="number" placeholder="Min" value={yields[days]?.min || 0}
                                        onChange={e => setEditForm({ ...editForm, yield_config: { ...yields, [days]: { ...yields[days], min: parseInt(e.target.value) } } })}
                                        className="border p-1 rounded w-full text-sm" />
                                    <span className="text-gray-400">-</span>
                                    <input type="number" placeholder="Max" value={yields[days]?.max || 0}
                                        onChange={e => setEditForm({ ...editForm, yield_config: { ...yields, [days]: { ...yields[days], max: parseInt(e.target.value) } } })}
                                        className="border p-1 rounded w-full text-sm" />
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <button type="button" onClick={() => setEditForm({ ...editForm, loot_table: [...loot, { item_id: '', rate: 0 }] })} className="bg-blue-100 text-blue-600 px-2 py-1 rounded text-xs font-bold">
                                    + Agregar Item
                                </button>
                            </div>
                            ...
                            {loot.map((l: any, idx: number) => (
                                <div key={idx} className="flex items-center gap-2 mb-2">
                                    <select
                                        ...
                                        onChange={e => {
                                            const newLoot = [...loot];
                                            newLoot[idx].item_id = e.target.value;
                                            setEditForm({ ...editForm, loot_table: newLoot });
                                        }}
                                        className="border p-1 rounded w-2/3 text-sm"
                                    >
                                    ...
                                    <input
                                        ...
                                        onChange={e => {
                                            const newLoot = [...loot];
                                            newLoot[idx].rate = parseInt(e.target.value);
                                            setEditForm({ ...editForm, loot_table: newLoot });
                                        }}
                                        className="border p-1 rounded w-1/3 text-sm"
                                    />
                                    <button type="button" onClick={() => {
                                        const newLoot = loot.filter((_: any, index: number) => index !== idx);
                                        setEditForm({ ...editForm, loot_table: newLoot });
                                    }} className="text-red-500 text-xl font-bold">×</button>
                                </div>
                            ))}
                        </div>

                        <div>
                            <label className="text-xs font-bold text-gray-500">URL Imagen Fondo Raid</label>
                            <input placeholder="URL" value={editForm.image_url || ''} onChange={e => setEditForm({ ...editForm, image_url: e.target.value })} className="border p-2 rounded w-full" />
                        </div>
                    </div>
                );
        }
